...
In many economic applications, it's necessary to decompose the time series into trend and cycle in order to relate the observed data with its theoretical counterpart. A classic example is the GDP series, where the trend component is often used as the empirical counterpart for potential GDP and the cycle component as the empirical counterpart for the output gap. In non-economic applications, decomposition may be employed simply to make time series smoother by removing transitory events.
